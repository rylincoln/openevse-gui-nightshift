...
<script lang="ts">
  ...
  async function startExpand(): Promise<void> {
    pendingExpand = false
    if (expanding) return
  ...
  let chipLabel = $derived.by(() => {
    const c: Partial<ConfigState> = $config_store ?? {}
  ...
  async function startFormat(): Promise<void> {
    pendingFormat = false
  ...
  let mem: Partial<Status> = $derived($status_store ?? {})
  let memSupported = $derived(mem.heap_largest !== undefined)
  let lvglSupported = $derived(mem.lv_used_max !== undefined)

  type Tone = 'default' | 'ok' | 'warn' | 'error'
  const TONE_CLASS: Record<Tone, string> =
    { default: 'text-text', ok: 'text-accent', warn: 'text-warning', error: 'text-error' }
  ...
  function heapTone(bytes: number | undefined): Tone {
    if (bytes == null) return 'default'
    if (bytes < 12 * 1024) return 'error'
    if (bytes < 20 * 1024) return 'warn'
    return 'default'
  }
  function stackTone(bytes: number | undefined): Tone {
    ...
    return !!bytes && bytes > 0 && bytes < 1024 ? 'warn' : 'default'
  }
  // Render the 0 sentinel (and missing fields) as "no reading yet", not "0 B".
  let stackValue = (bytes: number | undefined) => (bytes ? formatBytes(bytes) : '—')
  ...
  let resetTone: Tone = $derived(
    mem.reset_reason_name && !SOFT_RESET.has(mem.reset_reason_name) ? 'warn' : 'default',
  )
  let wsReapedTone: Tone = $derived((mem.ws_reaped ?? 0) > 0 ? 'warn' : 'default')
  ...
  let pct = (v: number | undefined) => (v == null ? '—' : `${v}%`)
  ...
  const PROBE_LABELS: Record<number, string> =
    { 0: 'config.terminal.probe_buildstatus', 1: 'config.terminal.probe_serialize' }
  const PROBE_KEYS: {
    i: number
    maxKey: 'probe0_max' | 'probe1_max' | 'probe2_max' | 'probe3_max'
    nKey: 'probe0_n' | 'probe1_n' | 'probe2_n' | 'probe3_n'
  }[] = [
    { i: 0, maxKey: 'probe0_max', nKey: 'probe0_n' },
    { i: 1, maxKey: 'probe1_max', nKey: 'probe1_n' },
    { i: 2, maxKey: 'probe2_max', nKey: 'probe2_n' },
    { i: 3, maxKey: 'probe3_max', nKey: 'probe3_n' },
  ]
  let probes = $derived(
    PROBE_KEYS
      .map(({ i, maxKey, nKey }) => ({ i, max: mem[maxKey], n: mem[nKey] }))
      .filter((p): p is { i: number; max: number | undefined; n: number } => !!p.n && p.n > 0),
  )
  ...
  let crash = $state<CrashSummary | null>(null)
  let pendingClear = $state(false) // confirmation dialog open
  let clearing = $state(false)

  async function loadCrash(): Promise<void> {
    const res = await serialQueue.add(() => httpAPI<CrashSummary>('GET', '/debug/crash'))
  ...
  let hex = (v: number | string | null | undefined) =>
    (v == null ? '—' : typeof v === 'number' ? '0x' + v.toString(16) : String(v))
  ...
  function downloadCrashSummary(): void {
    if (!crash) return
  ...
  async function clearCrash(): Promise<void> {
    pendingClear = false
    if (clearing) return
    clearing = true
    try {
...
<ConfigPage title={$_('config.pages.terminal')}>
    ...
    {#if sd || sdBusy || formatting}
      <div class="mt-4 rounded-xl border border-border p-3">
        ...
        {:else}
          ...
          <Button label={$_('config.terminal.sd_format_button')} variant="ghost" onclick={() => (pendingFormat = true)} />
        {/if}
      </div>
    {/if}
  ...
  {#if memSupported}
    <ConfigSection title={$_('config.terminal.memory')}>
      ...
      <ReadOnlyRow label={$_('config.terminal.ws_reaped')} value={mem.ws_reaped} tone={wsReapedTone} />
...
<Modal visible={pendingFormat} onclose={() => (pendingFormat = false)}>
  ...
  <div class="flex gap-2">
    ...
    <Button label={$_('config.terminal.sd_format_confirm_no')} variant="ghost" onclick={() => (pendingFormat = false)} />
  </div>
</Modal>
